...
#define otaclient mp3client // OTA uses mp3client for connection to host
    ...
    //**************************************************************************************************
    void
    displaytime(const char *str, uint16_t color = 0xFFFF);
...
void setup()
{
  ...
  if (index_html_version < 180102)
    dbgprint(wvn, "index");
  if (mp3play_html_version < 180918)
    dbgprint(wvn, "mp3play");
  if (defaultprefs_version < 180816)
    dbgprint(wvn, "defaultprefs");
  if (search_html_version < 210215)
    dbgprint(wvn, "searchprefs");
